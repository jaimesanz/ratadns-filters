...
class TestQueriesWithUnderscoredName(unittest.TestCase):
    ...
    def dataExample(self):
        ...
        data.putInformation('problematicQNames', {
            'www.ni_c.cl',
            'www.nic_labs.cl',
            'www._jerry_.cl',
            'www._pinky.cl'})
        ...
        # Answers
        data.addPacket({'dest': 'encrypted(ip1)',
                        'source': 'encrypted(dnsip1)',
                        'flags': '8000',
                        'queries': [
                            {'qname': 'www.ni_c.cl', 'qtype': '1'}]})
        data.addPacket({'dest': 'encrypted(ip2)',
                        'source': 'encrypted(dnsip2)',
                        'flags': '8000',
                        'queries': [
                            {'qname': 'www.niclabs.cl', 'qtype': '2'}]})
        data.addPacket({'dest': 'encrypted(ip3)',
                        'source': 'encrypted(dnsip3)',
                        'flags': '8000',
                        'queries': [
                            {'qname': 'www.jerry.cl', 'qtype': '5'}]})
        data.addPacket({'dest': 'encrypted(ip4)',
    ...
    def dataDifferenCase(self):
        ...
        data.addPacket({'dest': 'encrypted(dnsip1)',
                        ...
                        'queries': [
                            {'qname': 'www.ni_c.cl', 'qtype': '1'}]})
        expectedList.append({'server': 'encrypted(dnsip1)',
                             'sender': 'encrypted(ip1)',
                             'query': {
                                 'qname': 'www.ni_c.cl', 'qtype': '1'}})
    ...
    def dataRepeatError(self):
        ...
        for i in range(2):
            data.addPacket({'dest': 'encrypted(dnsip2)',
                            ...
                            'queries': [
                                {'qname': 'www.nic_labs.cl', 'qtype': '2'}]})
            expectedList.append({'server': 'encrypted(dnsip2)',
                                 'sender': 'encrypted(ip2)',
                                 'query': {
    ...
    def test_rightFormat(self):
        ...
        self.assertTrue('query' in someQuery)
        self.assertTrue('sender' in someQuery)
        self.assertTrue('server' in someQuery)

        queryItself = someQuery['query']

        self.assertTrue('qtype' in queryItself)
        self.assertTrue('qname' in queryItself)
    ...
    def test_dataDifferenCase(self):
        ...
        self.assertTrue(expectedProblematic in result)
        self.assertItemsEqual(example.expectedValue(
            expectedProblematic), result[expectedProblematic])
    ...
    def test_reset(self):
        ...
        for i in range(2):
            ...
            for source in expectedProblematic:
                self.assertTrue(source in result)
                self.assertEquals(
                    example.expectedValue(source), result[source])

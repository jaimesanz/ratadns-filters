...
from prers import PacketsQueriesAndAnswersPerSecond
...
class TestPacketsQueriesAndAnswersPerSecond(unittest.TestCase):
    ...
    def test_data_example(self):
        ...
        self.assertLessEqual(nq / (after_end_time - before_init_time),
                             result['qps'])
        self.assertGreaterEqual(nq / (before_end_time - after_init_time),
                                result['qps'])

        self.assertLessEqual(na / (after_end_time - before_init_time),
                             result['aps'])
        self.assertGreaterEqual(na / (before_end_time - after_init_time),
                                result['aps'])

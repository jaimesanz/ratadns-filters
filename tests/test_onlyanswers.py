...
class TestOnlyAnswers(unittest.TestCase):
    ...
    def dataExample(self):
        ...
        for i in range(4) :
            data.addPacket({'flags': '8000', 'queries' : [{'qname' : 'www.niclabs.cl'}]})
            listOfAnswers.append({'flags': '8000', 'queries' : [{'qname' : 'www.niclabs.cl'}]})

        for i in range(3) :
            data.addPacket({'flags': '8000', 'queries' : [{'qname' : 'www.uchile.cl'}]})
            listOfAnswers.append({'flags': '8000', 'queries' : [{'qname' : 'www.uchile.cl'}]})
        ...
        for i in range(2) :
            ...
            listOfAnswers.append({'flags': '8000', 'queries' : [{'qname' : 'www.pinky.cl'}]})

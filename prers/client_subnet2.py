    ...
    def __call__(self, p):
        if not p.is_answer():
            ...
            if self.is_non_auth_tld(p):
                # non-auth-tld: when the TLD is not one of the IANA-approved
                # TLDs.
            ...
            if p.source_port == 0 and p.transport_protocol == "udp":
                # src-port-zero: when the UDP message's source port equals
                # zero.
                self.incr_count("src-port-zero", source_ip)
                is_ok = False
            # if is_malformed(p): NOT IMPLEMENTED: this can't be done because
            # rata filters malformed queries, so they never get to this point
            # of the process
            if is_ok:
                # ok: not bogus.
